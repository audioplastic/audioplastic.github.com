...
The architecture of the BioAid algorithm is based on a [computational model of the auditory periphery](http://www.essex.ac.uk/psychology/department/HearingLab/modelling.html), developed  in the hearing research laboratory at the University of Essex. This model has undergone refinements over a time period spanning four decades. Therefore, it would be unwise to describe it in detail in this blog post! However, an overview can be given that describes the processes most relevant to the design of BioAid.
